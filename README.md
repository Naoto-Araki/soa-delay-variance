...
- 4つの実験条件
  - A: 統制（遅延なし）
  - B: 学習阻害（task=μ_delay, ans=0）
  - C: 学習促進（task=μ_delay, ans=μ_delay）
  - D: カスタム比率（task=μ_delay, ans=ratio×μ_delay、比率は0/0.5/0.75/1.0から選択可能）
...
**データ保存：** `data_continuous/{参加者ID}_condition{A/B/C/D}_{日時}.csv`
